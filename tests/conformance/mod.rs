...
use apcore::utils::{
    calculate_specificity, guard_call_chain_with_repeat, match_pattern,
...
fn conformance_version_negotiation() {
    ...
    for tc in fixture["test_cases"].as_array().unwrap() {
        ...
        if tc.get("expected_error").is_some() {
            ...
            assert!(result.is_ok(), "FAIL [{}]: expected Ok({}) but got {:?}", id, expected, result);
            assert_eq!(
                result.unwrap(),
                expected,
        ...
        match action {
            "register" => {
                ...
                if tc.get("expected_error").is_some() {
                    ...
                    assert!(result.is_ok(), "FAIL [{}]: expected Ok but got {:?}", id, result);
                }
            }
            "register_sequence" => {
                ...
                for (idx, step) in steps.iter().enumerate() {
                    ...
                    if is_last && has_error {
                        assert!(result.is_err(), "FAIL [{}]: expected error on last step", id);
                    } else {
