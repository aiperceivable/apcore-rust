...
- **`ACL::validate_conditions() -> Vec<ConditionValidationFinding>` (spec v1.22.0 §6.1.2 rule 3, §6.1.3, apcore#100).** The explicit validation entry point §6.1.2 makes a MUST: it reports every rule referencing a condition key that does not resolve, including keys nested inside `$or` / `$not`, so a deployment can assert on the result once handler registration is complete. Pure read — no mutation, no registration, no audit event — with findings ordered by rule index then ascending by key. Each finding carries `rule_index`, `condition_key`, `effect`, and **two** registry flags. `sync_registered` and `async_registered` are deliberately not collapsed into one boolean: `async_check()` consults the async registry and falls back to the sync one while `check()` consults only the sync one, so a key registered *only* as an async handler is a working condition on one path and an unevaluable one on the other, and a finding IS emitted for it (§6.1.3 rule 2).
...
- **`ExecutionPolicy::resolve_with_call_site(module_id, annotations, arguments, context)` (spec v1.24.0 §7.9.6, apcore#102).** Policy resolution now *receives* the call site. Added as a second method rather than by widening `resolve`, which §7.9.6 rule 5 explicitly permits "where extending the existing signature would break its public API" — `resolve` is `pub` on a published crate, Rust has neither default arguments nor overloading, and rule 2 forbids the new inputs from changing any verdict, so widening would break every external caller for no behavioural gain. **The built-in pattern rules MUST NOT consult the call site**, and do not: `resolve_with_call_site` records the call site's *shape* on the trace span (never argument values) and forwards to `resolve`, so a rule set's verdict stays a function of module ID and annotations alone and remains reproducible from the policy document. A differential test asserts bit-for-bit equality across a matrix of policies, modules, annotations and call sites. Note that `arguments` here have **not** been schema-validated: the approval gate is Step 5 and input validation is Step 7, so a host reading them must treat them as untrusted. Wired at both call sites — the Step-5 gate in `builtin_steps.rs` and `Executor::validate`'s preflight, so the two cannot report different verdicts.
...
- **BREAKING (security): an unevaluable ACL condition no longer silently disables a `deny` rule (spec v1.22.0 §6.1.1 / §6.3 / §6.3.1 / §6.5, apcore#100).** A condition is **unevaluable** in exactly three situations — the key has no registered handler, the handler panicked, or the handler was asynchronous and not ready on the synchronous `check()` path — and all three previously produced a plain `false`. A `deny` rule carrying a single misspelled key (`role:` for `roles:`) was therefore decoration: the rule did not match, evaluation fell through to the next rule and then to `default_effect`, and with `default_effect: allow` the call went through. §6.1.1 now requires an unevaluable condition to resolve the rule **toward refusing access**: a `deny` rule takes effect and the call is denied; an `allow` rule still does not match and still MUST NOT grant, so the permissive direction is unchanged.

  Applied on **both** paths — `matches_rule` and `matches_rule_async` are separate code, and the async-only-handler-on-the-sync-path case (§6.1.1 situation 3) was specified as "unsatisfied" through spec v1.21.0, which left a `deny` rule guarded by such a handler inert by a second route. Deliberately **not** changed: §6.5's "conditions present but no context provided" stays an ordinary non-match, because calling with no context is a legitimate shape for external entry points, not a misconfiguration; treating it as a failure would flip the decision for every `@external` call meeting a conditional `deny` rule.

  `conformance/fixtures/acl_handler_error.json` still pins the OLD expectation for one case (`throwing_handler_does_not_flip_default_allow_to_deny_unsafely`). The corrected fixture is staged in the spec repo at `planning/acl-unevaluable-conditions/staged-fixtures/` and lands once all three SDK drivers do, so CI does not go red across every repository during the rollout. `tests/test_acl_handler_error_conformance.rs` overrides that single case by ID and turns the override off automatically when the corrected fixture appears.

- **BREAKING: `ACL::evaluate_conditions` returns `ConditionOutcome`, not `bool`.** §6.3 makes carrying the distinction a MUST — "an implementation whose evaluator returns a plain boolean MUST carry the distinction some other way (a sentinel, an out-parameter, a typed result)". A typed result is the option that cannot be ignored at the call site. `ACL::evaluate_conditions_async` keeps its `bool` return as a façade for callers that only want "was it satisfied"; `ACL::evaluate_conditions_async_outcome` and `acl_handlers::evaluate_conditions_async_outcome` are the three-valued forms. Both evaluators now evaluate **every** child rather than short-circuiting: §6.1.1 permits either, and explicitly allows evaluating every child "for deterministic diagnostics" — which is what makes `handler_error` list all unevaluable keys instead of whichever one map iteration reached first. The decision is identical either way, since an outright `Unsatisfied` still wins a conjunction.
...
- **`ACL::load`, `ACL::try_new` / `ACL::new` and `ACL::add_rule` warn for unresolvable condition keys (spec v1.22.0 §6.1.2, apcore#100).** Each entry point that accepts rules now checks every referenced condition key — including keys nested inside `$or` / `$not` — against the sync handler registry, and warns for each that does not resolve, naming the rule index, the key and the rule's `effect`. The `effect` is in the message because a misconfigured `deny` rule is the consequential case. `add_rule` performed no validation at all before this; §6.1.2 rule 4 makes runtime insertion an entry point that MUST be covered, not just file loading. Loading and construction still **succeed**: `register_condition()` writes to a runtime, process-wide registry and `acl.root` discovery commonly runs during framework bootstrap ahead of application code, so failing here would reject valid configurations on ordering alone.

- **`AuditEntry.handler_error` reports every unevaluable condition, ordered lexicographically by condition key (spec v1.22.0 §6.1.1 rule 2, §6.3.1).** The per-call slot held one message and the last writer won. It now accumulates into a `BTreeMap` keyed by condition key and joins the messages with `"; "`. Lexicographic order rather than evaluation order because the two are not the same across languages — `serde_json`'s map is ordered while Python `dict` and JavaScript objects preserve insertion order, so "the first one encountered" would write a different key into the audit log for the same rule in different SDKs. `report_handler_error` keeps its free-form signature and derives the sort key from the conventional `"{key}: {reason}"` prefix; SDK-internal call sites pass the key explicitly.
